...
class App:
    def __init__(self, window, window_title, video_source=0, labelTarget=None):
        ...
        self.task = window_title.split("\\")[-3]
        ...
        self.frame_stride = int(self.vid.fps/3)
        ...
        self.back_button = Button(window,text="Go Back",width=15 , font='sans 15',command=self.back)
        self.back_button.pack(in_=self.bottom2,anchor=CENTER)
    ...
    def saveToFile(self):
        ...
        while(str(row) in record):
            ...
            if (self.task == "Suturing") or (self.task == "Needle_Passing"):
                o.write(str(needleStates.index(self.N.get())) + " ") # needle state
                o.write(str(threadStates.index(self.T.get())))       # thread state
            elif self.task == "Knot_Tying":
                o.write(str(cLoopStates.index(self.C.get())) + " ") # c loop state
                o.write(str(knotStates.index(self.K.get())))       # knot state
